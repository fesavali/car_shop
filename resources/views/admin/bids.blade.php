...
<!-- show error messages -->
  ...
  <div class="card" style="width: 18rem; ">
      <img src="{{ url('public/images/' . json_decode($bid->images, true)[1]) }}" class="card-img" alt="Car Image" style="height: 13rem;"/>
      </div>
  </div>
  ...
  <tbody>
    <tr>
      <th>Car Id: <b>{{ $bid->carId }}</b></th>
      <th>Plate No. <b>{{ strtoupper($bid->vin); }}</b></th>
      <th>Package: <b>{{ $bid->package }}</b></th>
    </tr>
    <tr>
      <th>Transaction code: <b>{{ strtoupper($bid->trans_id); }}</b></th>
      <th>Dealer Phone: <b>{{ strtoupper($bid->phone); }}</b></th>
      <th>Email: <b>{{ $bid->email }}</th>
    </tr>
    <tr>
      <th>Dealer Name: <b>{{ strtoupper($bid->firstname); }}</b></th>
    </tr>
  </tbody>
</table>
</div>
...
@endif
  ...
  <a href="{{ route('bidPage', $admin->id) }}" class="btn btn-primary">New Bid</a>
</br>
</br>
